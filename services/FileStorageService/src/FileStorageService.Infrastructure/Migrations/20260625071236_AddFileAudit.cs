...
{
    /// <inheritdoc />
    public partial class AddFileAudit : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.CreateTable(
                ...
                constraints: table =>
                {
                    table.PrimaryKey("PK_file_audit_logs", x => x.id);
                });

            migrationBuilder.CreateTable(
                name: "file_audit_outbox",
                columns: table => new
                {
                    id = table.Column<Guid>(type: "uuid", nullable: false),
                    event_id = table.Column<Guid>(type: "uuid", nullable: false),
                ...
                constraints: table =>
                {
                    table.PrimaryKey("PK_file_audit_outbox", x => x.id);
                });

            migrationBuilder.CreateIndex(
                name: "ix_file_audit_logs_action",
                table: "file_audit_logs",
                columns: new[] { "action_code", "created_at" });

            migrationBuilder.CreateIndex(
                name: "ix_file_audit_logs_target",
                table: "file_audit_logs",
                columns: new[] { "target_id", "created_at" });

            migrationBuilder.CreateIndex(
                name: "ux_file_audit_logs_event_id",
                table: "file_audit_logs",
                column: "event_id",
                unique: true);

            migrationBuilder.CreateIndex(
                name: "ix_file_audit_outbox_pending",
                table: "file_audit_outbox",
                columns: new[] { "status", "created_at" },
                filter: "status = 1");

            migrationBuilder.CreateIndex(
                name: "ux_file_audit_outbox_event_id",
                table: "file_audit_outbox",
                column: "event_id",
                unique: true);

            // #AUDIT-29 — trigger append-only soft mode cho file_audit_logs.
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION enforce_file_audit_logs_append_only_soft()
RETURNS TRIGGER AS $$
BEGIN
    IF (TG_OP = 'DELETE') THEN
        RAISE EXCEPTION 'file_audit_logs is append-only; DELETE is not allowed';
    END IF;
    IF (NEW.event_id IS DISTINCT FROM OLD.event_id
        ...
        RAISE EXCEPTION 'file_audit_logs business fields are immutable';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER file_audit_logs_append_only_soft
BEFORE UPDATE OR DELETE ON file_audit_logs
FOR EACH ROW EXECUTE FUNCTION enforce_file_audit_logs_append_only_soft();
");
        }
        ...
        {
            migrationBuilder.Sql(@"DROP TRIGGER IF EXISTS file_audit_logs_append_only_soft ON file_audit_logs;");
            migrationBuilder.Sql(@"DROP FUNCTION IF EXISTS enforce_file_audit_logs_append_only_soft();");
