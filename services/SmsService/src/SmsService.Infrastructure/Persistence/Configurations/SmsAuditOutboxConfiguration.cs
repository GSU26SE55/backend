...
/// <summary>Map <see cref="SmsAuditOutbox"/> → bảng <c>sms_audit_outbox</c> (Sprint audit #AUDIT-35).</summary>
public class SmsAuditOutboxConfiguration : IEntityTypeConfiguration<SmsAuditOutbox>
{
    public void Configure(EntityTypeBuilder<SmsAuditOutbox> b)
    {
        b.ToTable("sms_audit_outbox");
        b.HasKey(x => x.Id);
        ...
        b.HasIndex(x => x.EventId).IsUnique().HasDatabaseName("ux_sms_audit_outbox_event_id");
        b.HasIndex(x => new { x.Status, x.CreatedAt }).HasDatabaseName("ix_sms_audit_outbox_pending").HasFilter("status = 1");
