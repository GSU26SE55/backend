...
/// <summary>Map <see cref="NotificationAuditLog"/> → bảng <c>notification_audit_logs</c> (Sprint audit #AUDIT-34).</summary>
public class NotificationAuditLogConfiguration : IEntityTypeConfiguration<NotificationAuditLog>
{
    public void Configure(EntityTypeBuilder<NotificationAuditLog> b)
    {
        b.ToTable("notification_audit_logs");
        b.HasKey(x => x.Id);
        ...
        b.HasIndex(x => x.EventId).IsUnique().HasDatabaseName("ux_notification_audit_logs_event_id");
        b.HasIndex(x => new { x.ActionCode, x.CreatedAt }).HasDatabaseName("ix_notification_audit_logs_action");
