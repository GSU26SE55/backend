...
{
    /// <inheritdoc />
    public partial class AddBatteryAudit : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "battery_audit_logs",
                columns: table => new
                {
                ...
                constraints: table =>
                {
                    table.PrimaryKey("PK_battery_audit_logs", x => x.id);
                });

            migrationBuilder.CreateTable(
                name: "battery_audit_outbox",
                columns: table => new
                {
                    id = table.Column<Guid>(type: "uuid", nullable: false),
                    event_id = table.Column<Guid>(type: "uuid", nullable: false),
                ...
                constraints: table =>
                {
                    table.PrimaryKey("PK_battery_audit_outbox", x => x.id);
                });

            migrationBuilder.CreateIndex(
                name: "ix_battery_audit_logs_action",
                table: "battery_audit_logs",
                columns: new[] { "action_code", "created_at" });

            migrationBuilder.CreateIndex(
                name: "ix_battery_audit_logs_target",
                table: "battery_audit_logs",
                columns: new[] { "target_id", "created_at" });

            migrationBuilder.CreateIndex(
                name: "ux_battery_audit_logs_event_id",
                table: "battery_audit_logs",
                column: "event_id",
                unique: true);

            migrationBuilder.CreateIndex(
                name: "ix_battery_audit_outbox_pending",
                table: "battery_audit_outbox",
                columns: new[] { "status", "created_at" },
                filter: "status = 1");

            migrationBuilder.CreateIndex(
                name: "ux_battery_audit_outbox_event_id",
                table: "battery_audit_outbox",
                column: "event_id",
                unique: true);

            // #AUDIT-21 — trigger append-only soft mode cho battery_audit_logs (chặn DELETE + business field immutable).
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION enforce_battery_audit_logs_append_only_soft()
RETURNS TRIGGER AS $$
BEGIN
    IF (TG_OP = 'DELETE') THEN
        RAISE EXCEPTION 'battery_audit_logs is append-only; DELETE is not allowed';
    END IF;
    IF (NEW.event_id IS DISTINCT FROM OLD.event_id
        ...
        RAISE EXCEPTION 'battery_audit_logs business fields are immutable';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER battery_audit_logs_append_only_soft
BEFORE UPDATE OR DELETE ON battery_audit_logs
FOR EACH ROW EXECUTE FUNCTION enforce_battery_audit_logs_append_only_soft();
");
        }
        ...
        {
            migrationBuilder.Sql(@"DROP TRIGGER IF EXISTS battery_audit_logs_append_only_soft ON battery_audit_logs;");
            migrationBuilder.Sql(@"DROP FUNCTION IF EXISTS enforce_battery_audit_logs_append_only_soft();");
