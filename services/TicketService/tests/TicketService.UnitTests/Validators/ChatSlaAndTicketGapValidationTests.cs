...
/// </summary>
public class CustomerSchedulePeriodicMaintenanceCommandValidationTests
{
    private static CustomerSchedulePeriodicMaintenanceCommand Valid() => new()
    {
        TicketId = Guid.NewGuid(),
        CustomerId = Guid.NewGuid(),
        ScheduledStartAt = new DateTimeOffset(2026, 9, 15, 8, 0, 0, TimeSpan.FromHours(7))
    };
    ...
    /// <summary>CustomerId lấy từ token — thiếu nghĩa là lệnh không gắn được vào ai.</summary>
    [Fact]
    public async Task EmptyCustomerId_Fails()
    {
        ...
        (await c.ValidateAsync()).ListErrors.Should().Contain(e => e.Field == "CustomerId");
    }

    [Fact]
    public async Task DefaultScheduledStartAt_Fails()
    {
        var c = Valid();
        c.ScheduledStartAt = default;

        (await c.ValidateAsync()).ListErrors.Should().Contain(e => e.Field == "ScheduledStartAt");
    }

    [Fact]
    public async Task AllFieldsMissing_ReportsEveryField()
    {
        var r = await new CustomerSchedulePeriodicMaintenanceCommand().ValidateAsync();
