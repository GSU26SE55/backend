...
{
    /// <inheritdoc />
    public partial class AddTicketAudit : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.CreateTable(
                ...
                constraints: table =>
                {
                    table.PrimaryKey("PK_ticket_audit_outbox", x => x.id);
                });
            ...
            migrationBuilder.CreateIndex(
                name: "ix_ticket_audit_logs_correlation",
                table: "ticket_audit_logs",
                column: "correlation_id");

            migrationBuilder.CreateIndex(
                name: "ix_ticket_audit_logs_target",
                table: "ticket_audit_logs",
                columns: new[] { "target_id", "created_at" });

            migrationBuilder.CreateIndex(
                name: "ux_ticket_audit_logs_event_id",
                table: "ticket_audit_logs",
                column: "event_id",
                unique: true);

            migrationBuilder.CreateIndex(
                name: "ix_ticket_audit_outbox_pending",
                table: "ticket_audit_outbox",
                columns: new[] { "status", "created_at" },
                filter: "status = 1");

            migrationBuilder.CreateIndex(
                name: "ux_ticket_audit_outbox_event_id",
                table: "ticket_audit_outbox",
                column: "event_id",
                unique: true);

            // #AUDIT-25 — trigger append-only soft mode cho ticket_audit_logs.
            migrationBuilder.Sql(@"
CREATE OR REPLACE FUNCTION enforce_ticket_audit_logs_append_only_soft()
RETURNS TRIGGER AS $$
BEGIN
    IF (TG_OP = 'DELETE') THEN
        RAISE EXCEPTION 'ticket_audit_logs is append-only; DELETE is not allowed';
    END IF;
    IF (NEW.event_id IS DISTINCT FROM OLD.event_id
        ...
        RAISE EXCEPTION 'ticket_audit_logs business fields are immutable';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER ticket_audit_logs_append_only_soft
BEFORE UPDATE OR DELETE ON ticket_audit_logs
FOR EACH ROW EXECUTE FUNCTION enforce_ticket_audit_logs_append_only_soft();
");
        }
        ...
        {
            migrationBuilder.Sql(@"DROP TRIGGER IF EXISTS ticket_audit_logs_append_only_soft ON ticket_audit_logs;");
            migrationBuilder.Sql(@"DROP FUNCTION IF EXISTS enforce_ticket_audit_logs_append_only_soft();");
