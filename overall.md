...
|--------|-----------|----------|
| **`AuthService`** | ✅ Production-ready | Account/Role/Permission/Session/RefreshToken/AuditLog/LoginAttempt/OTP/Outbox + Admin CRUD + Google OAuth helper. Controllers: `AuthController`, `AccountsController`, `SessionsController`, `AdminAccountsController`, `AdminAuditLogsController`, `AdminPermissionsController`, `AdminRolesController` |
...
| **`FileStorageService`** | ✅ Có Api + Application + Infrastructure | MinIO backend, signed URLs |
...
| **Docker compose** | ✅ Done | postgres:16-alpine, redis:7, rabbitmq:3-management, minio, prometheus, grafana, loki, alertmanager |
...
| AuthService profile expansion (avatar, phone, skill) | 🟠 P1 | §7 | 0.5 sprint |
...
- .NET 8 LTS, C# 12
- EF Core 8, PostgreSQL 16 (sẽ chuyển sang `timescale/timescaledb:latest-pg16` — xem §6)
...
### 7.1. Quyết định
KHÔNG tách UserService — mở rộng `Account` entity hiện tại.

### 7.2. Fields bổ sung cho `Account`
| Field | Type | Note |
|-------|------|------|
| `AvatarFileId` | Guid? | reference FileStorageService |
| `PhoneNumber` | string(20)? | đã có sẵn từ OTP |
| `Address` | string(500)? | — |
| `BirthDate` | DateOnly? | — |
| `EmployeeCode` | string(20)? | Staff/Manager only |
| `Department` | string(100)? | — |
| `StaffSkills` | string(jsonb)? | Array `["LiFePO4", "NMC", "OnSite"]` — chỉ cho Staff |
| `MaxConcurrentTickets` | int? | Staff workload cap, default 10 |

### 7.3. Migration
```csharp
public partial class AddAccountProfileFields : Migration {
    protected override void Up(MigrationBuilder mb) {
        mb.AddColumn<Guid>("avatar_file_id", "accounts", nullable: true);
        mb.AddColumn<string>("address", "accounts", maxLength: 500, nullable: true);
        mb.AddColumn<DateOnly>("birth_date", "accounts", nullable: true);
        mb.AddColumn<string>("employee_code", "accounts", maxLength: 20, nullable: true);
        mb.AddColumn<string>("department", "accounts", maxLength: 100, nullable: true);
        mb.AddColumn<string>("staff_skills", "accounts", type: "jsonb", nullable: true);
        mb.AddColumn<int>("max_concurrent_tickets", "accounts", nullable: true, defaultValue: 10);
    }
    protected override void Down(MigrationBuilder mb) {
        mb.DropColumn("max_concurrent_tickets", "accounts");
        // ...
    }
}
```
...
GET    /api/v1/auth/staff/{id}/workload                  (internal — TicketService call)
PUT    /api/v1/auth/me/profile                           (mọi role update profile)
POST   /api/v1/auth/me/avatar                            (upload avatar)
```

### 7.5. New integration event
`AccountSkillsUpdatedEvent` → TicketService có thể cache skill matrix.
...
1. AuthService (đã done) + AddAccountProfileFields
2. BatteryService InitialBatterySchema (+ TimescaleDB extension)
3. TicketService InitialTicketSchema
4. NotificationService InitialNotificationSchema
5. FileStorageService (nếu có sửa)
...
- [ ] **Decision:** đổi postgres image sang `timescale/timescaledb:latest-pg16` — test rollback
- [ ] **Decision:** mở rộng Account entity (avatar, phone, address, employeeCode, department, staffSkills, maxConcurrentTickets) → migration `AddAccountProfileFields`
- [ ] Update CLAUDE.md memory + tài liệu API contract initial cho FE team
...
1. `docker compose up -d --build` chạy tất cả service xanh trong < 60s.
...
- [ ] Account profile expansion (avatar, phone, skill) — §7
...
| Q-05 | Account profile fields: mở rộng hay tách UserService? | **Mở rộng `Account`** |
...
│   └── SlaBreachedEvent.cs
├── Account/
│   └── AccountSkillsUpdatedEvent.cs           (new)
└── Notification/
...
Nếu hôm nay là 2026-05-12, branch hiện tại `feat/retrytimeout`:

1. **Hôm nay/ngày mai:** Merge PR #47 sau khi review pass.
2. **Tuần này:** Hoàn tất Sprint 1 decisions:
   - Đổi postgres image sang `timescaledb:latest-pg16` trên 1 branch test → run AuthService migrations → verify nothing breaks.
   - Tạo migration `AddAccountProfileFields` cho AuthService.
   - Viết API contract doc draft cho FE team start Sprint 2.
...
| ADR-014 | Account profile extension (vs separate UserService) |
...
sleep 5
for db in auth_db battery_db ticket_db notification_db; do
...
| EC-23 | Cross-timezone Customer (Mỹ) vs Staff (VN) | Tất cả timestamp lưu UTC, FE convert theo `Account.TimeZone` | Đã có TimeZone in NotificationPreference, expand to Account |
...
→ { "fileId": "...", "status": "Processing", "originalUrl": "..." }
