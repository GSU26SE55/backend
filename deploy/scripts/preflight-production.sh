...
TICKET_PERIODIC_MAINTENANCE_LEAD_DAYS="$(read_env TICKET_PERIODIC_MAINTENANCE_LEAD_DAYS)"
TICKET_PERIODIC_MAINTENANCE_OVERDUE_WINDOW_DAYS="$(read_env TICKET_PERIODIC_MAINTENANCE_OVERDUE_WINDOW_DAYS)"
TICKET_PERIODIC_MAINTENANCE_REMINDER_TIME="$(read_env TICKET_PERIODIC_MAINTENANCE_REMINDER_TIME)"
TICKET_PERIODIC_MAINTENANCE_POLL_INTERVAL_SECONDS="$(read_env TICKET_PERIODIC_MAINTENANCE_POLL_INTERVAL_SECONDS)"
TICKET_PERIODIC_MAINTENANCE_BATCH_SIZE="$(read_env TICKET_PERIODIC_MAINTENANCE_BATCH_SIZE)"
SLA_BUSINESS_HOURS_TIME_ZONE_ID="$(read_env SLA_BUSINESS_HOURS_TIME_ZONE_ID)"
SLA_BUSINESS_HOURS_START="$(read_env SLA_BUSINESS_HOURS_START)"
SLA_BUSINESS_HOURS_END="$(read_env SLA_BUSINESS_HOURS_END)"
SLA_BUSINESS_HOURS_WORKING_DAYS_0="$(read_env SLA_BUSINESS_HOURS_WORKING_DAYS_0)"
SLA_BUSINESS_HOURS_WORKING_DAYS_1="$(read_env SLA_BUSINESS_HOURS_WORKING_DAYS_1)"
SLA_BUSINESS_HOURS_WORKING_DAYS_2="$(read_env SLA_BUSINESS_HOURS_WORKING_DAYS_2)"
SLA_BUSINESS_HOURS_WORKING_DAYS_3="$(read_env SLA_BUSINESS_HOURS_WORKING_DAYS_3)"
SLA_BUSINESS_HOURS_WORKING_DAYS_4="$(read_env SLA_BUSINESS_HOURS_WORKING_DAYS_4)"
SLA_BUSINESS_HOURS_WORKING_DAYS_5="$(read_env SLA_BUSINESS_HOURS_WORKING_DAYS_5)"
SLA_BUSINESS_HOURS_WORKING_DAYS_6="$(read_env SLA_BUSINESS_HOURS_WORKING_DAYS_6)"
export KUBECONFIG
...
: "${TICKET_PERIODIC_MAINTENANCE_LEAD_DAYS:?TICKET_PERIODIC_MAINTENANCE_LEAD_DAYS is required}"
: "${TICKET_PERIODIC_MAINTENANCE_OVERDUE_WINDOW_DAYS:?TICKET_PERIODIC_MAINTENANCE_OVERDUE_WINDOW_DAYS is required}"
: "${TICKET_PERIODIC_MAINTENANCE_REMINDER_TIME:?TICKET_PERIODIC_MAINTENANCE_REMINDER_TIME is required}"
: "${TICKET_PERIODIC_MAINTENANCE_POLL_INTERVAL_SECONDS:?TICKET_PERIODIC_MAINTENANCE_POLL_INTERVAL_SECONDS is required}"
: "${TICKET_PERIODIC_MAINTENANCE_BATCH_SIZE:?TICKET_PERIODIC_MAINTENANCE_BATCH_SIZE is required}"
: "${SLA_BUSINESS_HOURS_TIME_ZONE_ID:?SLA_BUSINESS_HOURS_TIME_ZONE_ID is required}"
: "${SLA_BUSINESS_HOURS_START:?SLA_BUSINESS_HOURS_START is required}"
: "${SLA_BUSINESS_HOURS_END:?SLA_BUSINESS_HOURS_END is required}"
: "${SLA_BUSINESS_HOURS_WORKING_DAYS_0:?SLA_BUSINESS_HOURS_WORKING_DAYS_0 is required}"
: "${SLA_BUSINESS_HOURS_WORKING_DAYS_1:?SLA_BUSINESS_HOURS_WORKING_DAYS_1 is required}"
: "${SLA_BUSINESS_HOURS_WORKING_DAYS_2:?SLA_BUSINESS_HOURS_WORKING_DAYS_2 is required}"
: "${SLA_BUSINESS_HOURS_WORKING_DAYS_3:?SLA_BUSINESS_HOURS_WORKING_DAYS_3 is required}"
...
do
  [[ "${positive_integer}" =~ ^[1-9][0-9]*$ ]] || {
    printf 'periodic-maintenance numeric values must be positive integers\n' >&2
    exit 1
  }
done
for time_value in \
...
do
  [[ "${time_value}" =~ ^([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$ ]] || {
    printf 'business time values must use a valid HH:MM:SS value: %s\n' \
      "${time_value}" >&2
    exit 1
  }
done
...
do
  [[ "${time_zone_id}" =~ ^[A-Za-z0-9._+-]+(/[A-Za-z0-9._+-]+)+$ \
    && "${time_zone_id}" != *'..'* \
    && -f "/usr/share/zoneinfo/${time_zone_id}" ]] || {
    printf 'invalid or unavailable IANA time-zone ID: %s\n' "${time_zone_id}" >&2
    exit 1
  }
done

# TicketService currently enforces the project-wide 07:00-17:00 calendar and
# all seven explicit days at startup. Fail during preflight instead of letting
# an invalid host.env turn the Deployment into a crash loop.
[[ "${SLA_BUSINESS_HOURS_START}" == '07:00:00' \
  && "${SLA_BUSINESS_HOURS_END}" == '17:00:00' ]] || {
  printf 'SLA business hours must remain 07:00:00-17:00:00\n' >&2
  exit 1
}
expected_working_days=(Sunday Monday Tuesday Wednesday Thursday Friday Saturday)
...
for day_index in "${!expected_working_days[@]}"; do
  [[ "${configured_working_days[day_index]}" == "${expected_working_days[day_index]}" ]] || {
    printf 'SLA working day %s must be %s, got %s\n' \
      "${day_index}" \
      "${expected_working_days[day_index]}" \
      "${configured_working_days[day_index]}" >&2
    exit 1
  }
done
